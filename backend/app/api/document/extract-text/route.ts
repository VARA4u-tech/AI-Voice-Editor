...
export async function POST(req: Request) {
  try {
    ...
    if (file.type !== 'application/pdf' && file.type !== 'application/octet-stream') {
      return NextResponse.json({ detail: 'Only PDF files are supported.' }, { status: 415 });
    }
    ...
    if (buffer.length === 0) {
      return NextResponse.json({ detail: 'Uploaded PDF is empty.' }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ detail: `Failed to parse PDF: ${e.message}` }, { status: 422 });
    }
  } catch (error: any) {
